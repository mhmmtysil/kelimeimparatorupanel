...
import { GetPanelUser, UpdateCategory } from "@/services/apiService";
...
const Page = () => {
  ...
  async function getUser() {
    var a = await GetPanelUser({
      userId: selectedId,
    });
    if (a.objectData) {
      setUser(a.objectData);
      setActive(a.objectData.isActive);
      setDeleted(a.objectData.isDeleted);
    }
  }
  useEffect(() => {
    if (user) {
            ...
            <div className="flex w-full flex-col gap-5.5 p-6.5">
              ...
              <div className="border-b border-stroke py-4 dark:border-strokedark">
                <h3 className="font-medium text-black dark:text-white">
                  {selectedId} ID'li Oyuncu'yu Düzenle
                </h3>
              </div>
              <form
                className="flex w-full flex-col gap-5.5 p-6.5"
              ...
              >
                ...
                <div>
                  <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                    OneSignal Bildirim Kimliği (ID'si):
                  </label>
                  <input
                    type="text"
                    {...register("oneSignalID")}
                    defaultValue={user?.oneSignalID ?? ""}
                    className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setValue("coins", Number(onlyNumbers));
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const onlyNumbers = e.target.value.replace(/\D/g, "");
                      setValue("spinCounts", Number(onlyNumbers));
                    }}
                    defaultValue={user?.spinCounts}
                    className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
                {/* Kelime Tahmini Hakkı */}
                <div>
                  <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                    Kelime Tahmini Hakkı:
                  </label>
                  <input
                    type="text"
                    {...register("wordleCount")}
                    onChange={(e) => {
                      const onlyNumbers = e.target.value.replace(/\D/g, "");
                      setValue("wordleCount", Number(onlyNumbers));
                    }}
                    defaultValue={user?.wordleCount}
                    className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
                {/* Adam Asmaca Hakkı */}
                <div>
                  ...
                  <input
                    type="text"
                    {...register("hangmanCounts")}
                    onChange={(e) => {
                      const onlyNumbers = e.target.value.replace(/\D/g, "");
                      setValue("hangmanCounts", Number(onlyNumbers));
                    }}
                    defaultValue={user?.hangmanCounts}
                    className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
                </div>
                ...
                <div>
                  <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                    ...
                    <span className="text-meta-1">
                      *Kapalı olursa bu oyuncu oyunda yeni bir kullanıcı
                      oluşturarak girebilir.
                    </span>
                  </label>
                ...
                <div>
                  <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                    Silinecek mi?
                    <span className="text-meta-1">
                      *Açık olursa bu oyuncu oyunda yeni bir kullanıcı
                      oluşturarak girebilir.
                    </span>
                  </label>
